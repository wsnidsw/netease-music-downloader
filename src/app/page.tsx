      ...
      <div className="glass-card w-full max-w-2xl p-5 sm:p-6 mb-8 animate-in">
        <div className="flex gap-3">
          <input
            ...
            className="glass-input flex-1 px-4 py-3 text-sm sm:text-base"
            disabled={loading}
          />
          <button
            onClick={handleParse}
            disabled={loading || !link.trim()}
            className="btn-primary px-6 py-3 text-sm sm:text-base whitespace-nowrap"
          >
            {loading ? (
          ...
          <div className="flex gap-4 sm:gap-5 mb-5">
            {songInfo.coverUrl ? (
              <img
                ...
                alt={songInfo.name}
                className="w-24 h-24 sm:w-32 sm:h-32 rounded-xl object-cover flex-shrink-0 shadow-lg"
              />
            ) : (
              <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-xl bg-white/5 flex-shrink-0 flex items-center justify-center text-white/20 text-4xl">
                ♪
              </div>
            )}
            <div className="flex flex-col justify-center min-w-0">
              <h2 className="text-lg sm:text-xl font-semibold text-white/90 truncate">
                {songInfo.name}
              </h2>
          ...
          <div className="border-t border-white/[0.06] pt-5">
            ...
            <div className="flex flex-wrap items-center gap-4 sm:gap-6 mb-4">
              <label className="flex items-center gap-2.5 cursor-pointer">
                <input
                  type="checkbox"
                  checked={downloads.audio}
                  disabled={!songInfo.audioUrl}
                  onChange={(e) => setDownloads((d) => ({ ...d, audio: e.target.checked }))}
                  className="checkbox-custom"
                />
                <span className={`text-sm ${songInfo.audioUrl ? "text-white/70" : "text-white/20 line-through"}`}>
                  MP3 音频
                </span>
              </label>
              <label className="flex items-center gap-2.5 cursor-pointer">
                <input
                  type="checkbox"
                  checked={downloads.cover}
                  onChange={(e) => setDownloads((d) => ({ ...d, cover: e.target.checked }))}
                  className="checkbox-custom"
                />
              ...
              <label className="flex items-center gap-2.5 cursor-pointer">
                <input
                  type="checkbox"
                  checked={downloads.lyric}
                  onChange={(e) => setDownloads((d) => ({ ...d, lyric: e.target.checked }))}
                  className="checkbox-custom"
                />
